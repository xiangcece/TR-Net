...
class TemporalEncoder(nn.Module):
    def __init__(
        ...
        self.linear_cur1 = nn.Linear(hidden_size * 2, 2048)
        self.linear_cur2 = nn.Linear(hidden_size * 2, 2048)
        self.linear_bef = nn.Linear(hidden_size, 2048)
        self.linear_aft = nn.Linear(hidden_size, 2048)
        self.linear_bef1 = nn.Linear(hidden_size, 2048)
        self.linear_aft1 = nn.Linear(hidden_size, 2048)
        self.linear_out = nn.Linear(hidden_size, 2048)
        
     
        self.attention = TemporalAttention(attention_size=2048, seq_len=3, non_linearity='tanh')
        self.attention21 = TemporalAttention(attention_size=2048, seq_len=16, non_linearity='tanh')
        self.attention1 = TemporalAttention(attention_size=2048, seq_len=3, non_linearity='tanh')
        self.attention2 = TemporalAttention(attention_size=2048, seq_len=7, non_linearity='tanh')
    
    def forward(self, x, is_train=False):
        # NTF -> TNF[32, 16, 256]
        scores21 = self.attention21(x) #[32, 16, 1]
        out2 = torch.mul(x, scores21[:, :, None])
        out21 = torch.sum(out2, dim=1) # N x 2048        
        ...
        x_bef = x[:, :self.mid_frame+1]
        x_aft = x[:, self.mid_frame:]
        ...
        # y_*: N x 2048
        y_cur0 = self.linear_cur(F.relu(y[self.mid_frame])) 
        y_bef0 = self.linear_bef(F.relu(y_bef[-2]))
        y_aft0 = self.linear_aft(F.relu(y_aft[-2])) 
        y_bef1 = self.linear_bef1(F.relu(y_bef[-1]))
        y_aft1 = self.linear_aft1(F.relu(y_aft[-1])) 
        y0 = torch.cat((y_bef1[:, None, :], y_cur0[:, None, :], y_aft1[:, None, :]), dim=1) #[32, 3, 2048]
        scores = self.attention1(y0)
        out0 = torch.mul(y0, scores[:, :, None])
        out0 = torch.sum(out0, dim=1)  # N x 2048
     
        
        y = torch.cat((y_bef0[:, None, :], out0[:, None, :], y_aft0[:, None, :]), dim=1) #[32, 3, 2048]
        
        scores = self.attention(y)
        out = torch.mul(y, scores[:, :, None])
        out = torch.sum(out, dim=1) +out21  # N x 2048
...
class TCMR(nn.Module):
    ...
    def forward(self, input, is_train=False, J_regressor=None):
        ...
        feature = feature.reshape(-1, feature.size(-1)).contiguous()
